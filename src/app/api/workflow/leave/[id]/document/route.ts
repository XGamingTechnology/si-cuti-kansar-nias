...
export function createLeaveDocumentHandler(factory = createWorkflowRuntime) {
  return async (
    request: Request,
    ...
    try {
      ...
      const annualBalances =
        leave.currentRevision.leaveType === "ANNUAL"
          ...
          : undefined;
      const official = await runtime.officials.resolve(
        leave.currentRevision.submittedAt,
      );
      const pdf = generateLeaveDocument(leave, [], variant, undefined, {
        annualBalances,
        authorizedOfficial: official
          ? {
...
export const GET = createLeaveDocumentHandler();
